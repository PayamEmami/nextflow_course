...
For example, here we are a channel and a process that gets an input the channel and prints the values.
...
Try the above code! Create two files, `main_11.nf`  and `nextflow_11.config`. and run the above example!
...
```{bash, attr.source='.numberLines',eval=FALSE}
nextflow run main_11.nf -c  nextflow_11.config --ppm_input 20
```
...
Create a file called `nextflow_12.config` with the following content:
...
Create a file called `main_12.nf`
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow main_12.nf -c nextflow_12.config --project "uppmax2022-2-10" --clusterOptions "-M snowy" 
```
...
We can later create a file named `nextflow_13.config` with the following content
...
We can now create another file ` main_13.nf` with the following content:
...
```{bash, attr.source='.numberLines',eval=FALSE}
nextflow main_13.nf -c nextflow_13.config 
```
...
```{bash, attr.source='.numberLines',eval=FALSE}
nextflow main_14.nf -c nextflow_14.config --project "uppmax2022-2-10" --clusterOptions "-M snowy" 
```
...
```{bash, attr.source='.numberLines',eval=FALSE}
nextflow main.nf -profile uppmax_singularity --project "uppmax2022-2-10" --clusterOptions "-M snowy" 
```
...
As said before, you can connect two processes using channels. 
...
file x from mzMLFiles
output:
file "${x.baseName}.featureXML" into outputChannel #1
...
In this example, the process `featureFinder` defines an output channel (#1) which is then read by `someotherprocess` to do further processing.


Create a file called `main_15.nf`
...
file x from mzMLFiles
output:
file "${x.baseName}.featureXML" into outputChannel
"cp $x ${x.baseName}.featureXML"
...
echo true
input:
file x from outputChannel.collect() #1
...
In this example, the process `featureFinder` defines an output channel (#1) which is then read by `someotherprocess` to do further processing. However, `someotherprocess` now has all the data at once because.
...
echo true
input:
file x from outputChannel.collect()
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow main_17.nf --inputmzml "/crex/proj/uppmax2022-2-10/metabolomics/mzMLData/*.mzML"
```
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow main_18.nf --inputmzml "/crex/proj/uppmax2022-2-10/metabolomics/mzMLData/*.mzML" --qc 100
```
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow run main_18.nf --inputmzml "/crex/proj/uppmax2022-2-10/metabolomics/mzMLData/*.mzML" --qc 100
```
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow run main_18.nf -resume --inputmzml "/crex/proj/uppmax2022-2-10/metabolomics/mzMLData/*.mzML" --qc 100
```
...
Here are two assignments for finish the Nextflow part of the course.
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
cp -r /crex/proj/uppmax2022-2-10/metabolomics/xcms_pipeline .
```
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow main.nf -profile uppmax --project "uppmax2022-2-10" --clusterOptions "-M snowy"
```
...
2. Since the process in this specific pipeline need a separate parameter file you will need three additional **file** channels
    1. The first one is used in the FeatureFinder process and is located here: `/crex/proj/uppmax2022-2-10/metabolomics/openms_params/FeatureFinder.ini`
    2. The second one is for the alignment process and is located here: `/crex/proj/uppmax2022-2-10/metabolomics/openms_params/featureAlignment.ini`
    3. The third one is for the linker process and is located here: `/crex/proj/uppmax2022-2-10/metabolomics/openms_params/featureLinker.ini`
